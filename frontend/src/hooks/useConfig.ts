...
/** Lekka wersja modelu do cache — bez pricing/architecture (oszczędność localStorage). */
function slimModelForCache(m: Model): Model {
  return {
    id: m.id,
    name: m.name,
...
function writeModelsCache(models: Model[]): void {
  const slim = models.map(slimModelForCache);
  createSafeStorage().setItem(MODELS_CACHE_KEY, JSON.stringify(slim));
}
  ...
  const query = useQuery<Model[]>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        let models = (list as Omit<Model, 'active'>[]).map((m) => ({ ...m, active: true }));
        ...
        writeModelsCache(deduplicatedModels);
