...
import { JudgmentsIcon } from "../Layout/RealisticIcons";
import { cn } from "../../utils/cn";
import { API_BASE } from "../../config";
import { SaosApiHelp } from "./SaosApiHelp";
import {
  JUDGMENTS_SHELL,
  LibraryHero,
  LibraryStatPill,
...
const INPUT_CLASS =
  "library-view-cell w-full rounded-xl px-4 py-3 text-xs font-semibold text-black outline-none focus:border-library-accent/45";

const LABEL_CLASS = "library-view-label not-italic block mb-1.5";
...
export const JudgmentsView: React.FC = () => {
  ...
  return (
    ...
    <div className="h-full w-full min-h-0 flex flex-col overflow-hidden px-4 sm:px-6 lg:px-8 pt-1 pb-3 sm:pb-4">
      <div className={JUDGMENTS_SHELL}>
        <LibraryHero
          variant="documents"
          ornament="SAOS · Precedensy"
          title="Orzecznictwo"
          subtitle="Wyszukiwanie orzeczeń sądowych — filtry, AI i zapis do dokumentacji."
          badge={
            <>
              <LibraryStatPill label="Wyniki" value={totalResults} />
              {filters.useAi && (
                <span className="rounded-xl border border-gold-primary/25 bg-gold-primary/10 px-3 py-2 text-[9px] font-black uppercase tracking-widest text-gold-bright font-outfit">
                  AI wł.
                </span>
              )}
            </>
        ...
        <LibraryToolbar className="space-y-3">
        <SaosApiHelp className="w-full" />
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className="px-3 py-2 rounded-lg library-view-cell text-[10px] font-black uppercase tracking-widest text-black flex items-center gap-2 font-outfit"
              >
                <Filter size={14} />
              ...
              <button
                ...
                className={cn(
                  "px-3 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 font-outfit",
                  filters.useAi ? "library-filter-active" : "library-view-cell text-black/50",
                )}
              >
                <Sparkles size={14} />
              ...
              <button
                ...
                className="px-3 py-2 rounded-lg library-view-cell text-[10px] font-black uppercase tracking-widest text-black/60 flex items-center gap-2 font-outfit"
              >
                <RefreshCw size={14} />
                Reset
              </button>
            ...
            <AnimatePresence initial={false}>
              {showAdvancedFilters && (
                  ...
                  exit={{ opacity: 0, y: -8 }}
                  className="p-4 lg:p-5 rounded-xl library-view-panel space-y-5"
                >
          ...
          <AnimatePresence>
            {activeOptimizedQuery && (
              <motion.div
                initial={{ opacity: 0, y: -8 }}
                animate={{ opacity: 1, y: 0 }}
                className="flex items-center gap-3 px-4 py-2 glass-prestige rounded-xl border-gold-primary/20"
              >
                <div className="flex gap-1">
                  {[1, 2, 3].map((index) => (
                    <motion.div
                      ...
                      className="w-1 h-1 rounded-full bg-gold-primary"
                    />
                  ))}
                </div>
                ...
                <span className="text-[10px] font-bold text-gold-deep italic">
                  "{activeOptimizedQuery}"
                </span>
              </motion.div>
            )}
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        <div className="h-full overflow-y-auto px-4 sm:px-5 py-4 custom-scrollbar">
          {errorMessage && (
          ...
          {results.length === 0 && !loading && (
            <div className="h-full min-h-[280px] flex flex-col items-center justify-center text-center library-view-panel py-12 px-6 mx-auto max-w-lg">
              <JudgmentsIcon className="w-16 h-16 mb-5 opacity-60" />
              <p className="font-profile-display text-xl font-semibold italic text-black">Uruchom wyszukiwanie SAOS</p>
              <p className="text-[12px] text-black/45 font-outfit mt-2 max-w-sm">
                Wpisz frazę lub rozwiń filtry zaawansowane — wyniki pojawią się poniżej.
              </p>
            </div>
          )}
            ...
            {results.map((item, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className={cn(
                      "judgments-result-card group relative p-4 sm:p-5 cursor-pointer",
                      selectedJudgment?.id === item.id && selectedJudgment?.source === item.source && "is-selected",
                    )}
                  >
          ...
          {selectedJudgment && (
            ...
            >
              <motion.div
                ...
                className="judgments-detail-modal w-full max-w-[min(100%,72rem)] h-[min(94vh,1200px)] max-h-[min(94vh,1200px)] rounded-2xl xl:rounded-[28px] bg-[#f8f6f1] border border-black/20 shadow-[0_30px_80px_rgba(0,0,0,0.35)] overflow-hidden flex flex-col"
              ...
              >
                <div className="relative p-5 border-b border-black/10 flex items-center justify-between bg-white/50">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="p-3 rounded-xl border border-gold-primary/50 bg-gold-primary/10 text-gold-deep hover:text-black hover:bg-gold-primary/20 shadow-[0_0_10px_rgba(212,175,55,0.35)] hover:shadow-[0_0_20px_rgba(212,175,55,0.55)] transition-all duration-200 disabled:opacity-35 disabled:shadow-none disabled:border-gold-primary/25"
                    ...
                    <button
                      ...
                      className="p-3 rounded-xl border border-gold-primary/40 bg-gold-primary/8 text-gold-deep hover:text-black hover:bg-gold-primary/15 shadow-[0_0_10px_rgba(212,175,55,0.25)] hover:shadow-[0_0_18px_rgba(212,175,55,0.45)] transition-all duration-200 disabled:opacity-35 disabled:shadow-none disabled:border-gold-primary/20"
                ...
                <div className="flex-1 overflow-y-auto p-5 lg:p-8 custom-scrollbar bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.8),_rgba(0,0,0,0.03))]">
                  <div className="bg-white/80 border border-black/10 rounded-2xl p-6 lg:p-9 shadow-[0_10px_35px_rgba(0,0,0,0.12)] space-y-8">
                    ...
                    <div className="p-5 glass-liquid-convex border-gold-primary/15 space-y-3 shadow-none">
                      <h4 className="text-[11px] font-black text-gold-deep uppercase tracking-widest flex items-center gap-2">
                        <Search size={14} />
