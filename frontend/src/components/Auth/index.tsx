...
export function AuthView() {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center sm:p-4 bg-black/80 backdrop-blur-xl">
            <motion.div 
                initial={{ opacity: 0, scale: 0.9, y: 20 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                className="w-full h-full sm:h-auto sm:max-w-md glass-prestige bg-(--bg-top) sm:rounded-4xl p-8 lg:p-12 border-gold-gradient shadow-[0_0_100px_rgba(255,215,128,0.15)] relative overflow-hidden flex flex-col justify-center sm:block"
            >
                {/* Decorative BG */}
                <div className="absolute top-0 right-0 w-32 h-32 bg-gold-primary/10 blur-3xl rounded-full -translate-y-1/2 translate-x-1/2" />
                ...
                <form onSubmit={handleAuth} className="space-y-6 relative z-10" autoComplete="off">
                    <div className="space-y-4">
                        <div className="relative group">
                            ...
                            <input 
                                ...
                                className="w-full bg-black/40 border border-gold-muted/30 rounded-2xl py-4 pl-12 pr-4 text-xs font-bold tracking-widest text-(--text-primary) focus:outline-hidden focus:border-gold-primary transition-all placeholder:text-slate-600"
                                required
                        ...
                        <div className="relative group">
                            ...
                            <input 
                                ...
                                className="w-full bg-black/40 border border-gold-muted/30 rounded-2xl py-4 pl-12 pr-12 text-xs font-bold tracking-widest text-(--text-primary) focus:outline-hidden focus:border-gold-primary transition-all placeholder:text-slate-600"
                                required
                    ...
                    <button 
                        type="submit"
                        disabled={loading}
                        className="w-full bg-gold-primary text-black font-black uppercase text-[10px] tracking-[0.3em] py-5 rounded-2xl shadow-gold hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2 group"
                    >
                        {loading ? (
