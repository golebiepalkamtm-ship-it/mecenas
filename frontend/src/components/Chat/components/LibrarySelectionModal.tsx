...
export function LibrarySelectionModal({ isOpen, onClose, onSelect, mode = 'all' }: LibrarySelectionModalProps) {
  ...
  const defaultTab: 'user' | 'rag' = mode === 'all' ? 'rag' : 'user';
  const [activeTab, setActiveTab] = useState<'user' | 'rag'>(defaultTab);
  const resolvedTab = mode === 'all' ? activeTab : defaultTab;
  
  const documents = useMemo(() => {
    if (resolvedTab === 'rag') return (rag.documents as KnowledgeDocument[]).map(d => ({ 
      id: d.id,
      name: d.name,
      chunks: d.chunks,
  ...
  }, [resolvedTab, rag.documents, library.documents]);

  const refresh = () => {
    if (resolvedTab === 'rag') rag.refresh();
  ...
  const handlePreview = async (doc: DocWithContent) => {
    ...
    if (!doc.content) {
      ...
      try {
        ...
        if (data.success) {
          setPreviewDoc((prev) =>
            prev?.id === doc.id ? { ...prev, content: data.content } : prev,
          );
          return;
        }
      } catch (err) {
        ...
        <>
          ...
          >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-sm font-black uppercase tracking-[0.3em] text-white">
                    {activeTab === 'rag' ? 'Biblioteka Akt RAG' : 'Twoje Dokumenty'}
                  </h2>
                  <p className="text-[9px] font-bold text-white/30 uppercase tracking-widest mt-1">
                    {activeTab === 'rag' ? 'Globalna Baza Wiedzy (LexMind)' : 'Repozytorium Twoich Dokumentów i Skanów'}
                  </p>
                </div>
              </div>
            ...
            <div className="px-6 py-4 flex flex-col md:flex-row gap-4 items-center bg-white/5 border-b border-white/10">
              {/* Tab Selector */}
              <div className="flex items-center gap-1 p-1 bg-black/40 rounded-xl border border-white/5 select-none shrink-0 w-full md:w-auto">
                <button
                  onClick={() => setActiveTab('user')}
                  className={cn(
                    "flex-1 md:flex-none px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2",
                    activeTab === 'user'
