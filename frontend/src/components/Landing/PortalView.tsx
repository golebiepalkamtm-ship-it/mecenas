...
function Background() {
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
      ...
      <div
        className="absolute -top-[20%] -left-[20%] w-full h-full rounded-full opacity-40 animate-aurora-slow"
        style={{
          // Softer gradient avoids the need for heavy blur filters
          background: "radial-gradient(circle, rgba(212,175,55,0.08) 0%, rgba(212,175,55,0.03) 40%, transparent 80%)",
        }}
      />
      <div
        className="absolute top-[10%] -right-[20%] w-[80%] h-[80%] rounded-full opacity-35 animate-aurora-medium"
        style={{
          background: "radial-gradient(circle, rgba(212,175,55,0.06) 0%, rgba(212,175,55,0.02) 40%, transparent 80%)",
        }}
      />
      <div
        className="absolute -bottom-[30%] left-[10%] w-[90%] h-[90%] rounded-full opacity-30 animate-aurora-fast"
        style={{
          background: "radial-gradient(circle, rgba(212,175,55,0.04) 0%, rgba(212,175,55,0.01) 40%, transparent 80%)",
        }}
      />
...
export function PortalView({ onBack, onLoginSuccess }: { onBack?: () => void; onLoginSuccess?: () => void }) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      setMessage({
        type: "error",
        text: "Baza danych wyłączona - tryb offline",
      });
      setLoading(false);
      return;
    }
    ...
    try {
      if (isSignUp) {
        ...
        if (error) throw error;
        // Przekierowanie do głównego interfejsu po pomyślnym zalogowaniu
        if (onLoginSuccess) {
          onLoginSuccess();
        }
      }
  ...
  return (
    <>
      <style>{`
        ...
        .animate-aurora-slow { animation: aurora-flow 60s infinite ease-in-out; }
        .animate-aurora-medium { animation: aurora-flow 45s infinite ease-in-out reverse; }
        .animate-aurora-fast { animation: aurora-flow 35s infinite ease-in-out; }
        .lx-panel { animation: lx-panel-pulse 8s ease-in-out infinite; }
        .lx-scan  { animation: lx-scan 12s linear infinite; }
      `}</style>
      ...
      {onBack && (
        <motion.button
          ...
          onClick={onBack}
          className="fixed top-8 right-8 z-50 flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/20 bg-white/5 text-white/70 hover:text-white hover:bg-white/10 transition-all duration-300"
        >
          <ArrowLeft size={16} />
          <span className="text-[10px] font-black uppercase tracking-[0.2em]">Wróć</span>
        </motion.button>
      )}
