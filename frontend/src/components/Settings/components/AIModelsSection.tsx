...
export function AIModelsSection({ onUpdateProfile, isSaving, successMsg }: Pick<SettingsViewProps, 'onUpdateProfile' | 'isSaving' | 'successMsg'>) {
  ...
  const adminEnabledModels = useMemo(() => readEnabledModels(), []);
  ...
  const groupedAndFilteredModels = useMemo(() => {
    ...
    allModels.forEach((m: Model) => {
      // FIRST: Filter by Admin-enabled models
      if (adminEnabledModels.length > 0 && !adminEnabledModels.includes(m.id)) {
        return;
      }
      ...
      if (!isModelVisibleForProviders(m, activeProviders)) {
        return;
      }
    ...
    return groups;
  }, [allModels, filterVendor, searchQuery, activeProviders, adminEnabledModels]);
          ...
          <div className="flex items-center gap-2 relative">
                ...
                {hoveredAction === 'clear' && (
                  ...
                  >
                    ...
                    <p className="text-[8px] leading-relaxed text-black/60 font-bold uppercase tracking-wider">
                      Usuwa wszystkie wybrane modele z Twojej listy ulubionych.
                    </p>
                    <div className="absolute bottom-full right-6 -mb-px w-2 h-2 bg-white border-l border-t border-black/10 rotate-45" />
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            <div className="relative">
              <button 
                ...
                onMouseEnter={() => setHoveredAction('ping')}
                onMouseLeave={() => setHoveredAction(null)}
              ...
              >
                <LexIcon name="ai" size={12} className={cn(isPinging && "animate-pulse")} />
              ...
              <AnimatePresence>
                {hoveredAction === 'ping' && (
                  ...
                  >
                    ...
                    <p className="text-[8px] leading-relaxed text-black/60 font-bold uppercase tracking-wider">
                      Wykonuje próbne połączenie do wszystkich wybranych modeli, aby sprawdzić ich czas odpowiedzi (ping).
                    </p>
                    <div className="absolute bottom-full right-10 -mb-px w-2 h-2 bg-white border-l border-t border-black/10 rotate-45" />
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
              ...
              <AnimatePresence>
                {hoveredAction === 'save' && (
                  ...
                  >
                    ...
                    <p className="text-[8px] leading-relaxed text-black/60 font-bold uppercase tracking-wider">
                      Zapisuje aktualny skład zespołu AI w Twoim profilu.
                    </p>
                    <div className="absolute bottom-full right-6 -mb-px w-2 h-2 bg-white border-l border-t border-black/10 rotate-45" />
                  </motion.div>
                )}
