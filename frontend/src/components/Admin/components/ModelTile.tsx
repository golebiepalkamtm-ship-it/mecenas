...
import { getModelSourceBadge } from "../../../utils/modelSource";
...
export function ModelTile({
  ...
  const sourceBadge = getModelSourceBadge(model);
  ...
  return (
    <motion.button
      ...
      onClick={onToggle}
      className={cn(
        'library-view-cell group relative flex items-center justify-between p-3 h-[65px] w-full text-left overflow-hidden transition-all duration-400',
        isEnabled
            ? 'border-gold-primary/35 bg-gold-primary/8 shadow-[0_8px_20px_rgba(180,120,40,0.12)]'
            : 'hover:bg-white/50 hover:border-black/12'
      )}
    >
        ...
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-2">
           ...
           <div className="flex items-center gap-1.5 mt-0">
            ...
            <span className="text-[6.5px] text-black/35 font-black uppercase tracking-[0.15em] font-outfit">
                {sourceBadge}
            </span>
            <div className="w-0.5 h-0.5 rounded-full bg-black/20" />
