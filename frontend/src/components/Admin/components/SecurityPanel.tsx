...
import { SectionHeading, AdminPanel } from "./Shared";

interface SecurityPanelProps {
  providers: { id: string; name: string; active: boolean; key: string }[];
              ...
              <div className="flex items-center gap-4 animate-in fade-in slide-in-from-right-2 bg-white/5 p-4 rounded-2xl border border-black/10">
                <div className="flex flex-col gap-2">
                  <input
                    ...
                    className="h-10 px-4 rounded-xl library-view-cell text-[10px] font-admin-mono text-black outline-none focus:border-gold-primary/45 w-64"
                  />
                  {detectedProvider && (
                ...
                <input
                  ...
                  className="h-10 px-4 rounded-xl library-view-cell text-[10px] font-black uppercase tracking-widest text-black outline-none focus:border-gold-primary/45 w-48 font-outfit"
                />
                ...
                <button
                  ...
                  className="h-10 px-6 rounded-xl bg-gold-primary text-black text-[10px] font-black uppercase tracking-widest hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50 border border-gold-deep/40 shadow-[0_8px_20px_rgba(180,120,40,0.3)] font-outfit"
                >
                  ZAPISZ
                </button>
               ...
               <button
                ...
                className="h-10 px-4 rounded-xl library-view-cell text-[10px] font-black uppercase tracking-widest text-black/65 inline-flex items-center gap-2 hover:text-black transition-all font-outfit"
              >
                <Plus size={14} />
                Nowy Klucz API
              </button>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
          {providers.map((p) => (
            <div
              key={p.id}
              className={cn(
                "group relative overflow-hidden rounded-xl border transition-all duration-400",
                p.active
                  ? "library-view-cell border-gold-primary/25"
                  : "bg-black/[0.03] border-black/[0.06] grayscale opacity-55",
              )}
            >
              <div className="px-6 py-3 flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative z-10">
                ...
                <div className="flex-1 max-w-2xl relative">
                  <input
                    ...
                    className="w-full h-10 library-view-cell px-12 text-[10px] font-admin-mono text-black placeholder:text-black/15 outline-none focus:border-gold-primary/40"
                  />
