...
const storePath = 'c:\\Users\\Marcin_Palka\\moj prawnik\\frontend\\src\\store\\useChatSettingsStore.ts';
let content = fs.readFileSync(storePath, 'utf8');

const newRoles = `    defender: \`[SYSTEM_ROLE: THE DEFENDER — NACZELNY ADWOKAT]
...
const newTasks = `    criminal_defense: \`[TASK: FULL_SPECTRUM_CRIMINAL_DEFENSE]
METODOLOGIA OBRONY:
...
const oldTasksDef = `    strategy: \`[TASK: STRATEGIC_WAR_ROOM]

PROCEDURA:
1. SYTUATIONAL ASSESSMENT: Co mamy? Czego nie mamy? Co zrobi przeciwnik?
2. OFFENSIVE/DEFENSIVE: Atakujemy czy budujemy obronę?
3. EVIDENCE INVENTORY: Lista dowodów "Must-Have" (art. 6 k.c. / art. 74 KPK)
4. ANTICIPATORY RESPONSE: Top 3 argumenty przeciwnika + kontrargumenty
5. MULTI-PATH PLANNING:
   PLAN A (optymalny) → krok po kroku
   PLAN B (jeśli A zawiedzie) → alternatywa
   PLAN C (awaryjny) → minimum damage control
6. TACTICAL TIMELINE: Harmonogram od wezwania przedsądowego do apelacji

CEL: Wygrać sytuację, nie tylko odpowiedzieć na pytanie.\``;

const newTasksDef = oldTasksDef + `,\n\n` + newTasks;

content = content.replace(oldTasksDef, newTasksDef);

const oldRoleDef = `    grandmaster: \`[PERSPEKTYWA: GRANDMASTER — Red Teamer / Devil's Advocate]

SPOSÓB MYŚLENIA:
Twoim jedynym celem jest brutalna, intelektualna destrukcja strategii. Grasz w drużynie PRZECIWNIKA.
Szukasz dziur, braku spójności i niedozwolonych optymistycznych założeń.

METODA:
1. ATAK NA ZAŁOŻENIA: Gdzie strategia opiera się na domysłach zamiast dowodów?
2. KONTRARGUMENTACJA: Jakie artykuły/orzeczenia zniweczą roszczenie?
3. SCENARIUSZ KATASTROFY: Najgorszy możliwy obrót spraw
4. PRZEWIDYWANIE: Dla każdego ruchu → reakcja przeciwnika → Twoja odpowiedź

ZASADA: Zero rozwiązań. Twoim celem jest NISZCZENIE strategii, nie naprawianie.

OUTPUT: Ataki na założenia → Kontrargumenty → Scenariusz katastrofy\``;

const newRoleDef = oldRoleDef + `,\n\n` + newRoles;

content = content.replace(oldRoleDef, newRoleDef);
...
// Update version
content = content.replace(/version: 12/g, 'version: 13');
content = content.replace(/lexmind-chat-persistent-settings-v12/g, 'lexmind-chat-persistent-settings-v13');
