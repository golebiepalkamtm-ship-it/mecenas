...
### 6. Indeksowanie i migracje

Indeksowanie bazy wiedzy odbywa się przez Supabase (Edge Functions / RPC). Migracje schematu:

```bash
supabase db push
# lub ręcznie: SQL z supabase/migrations/
```
...
**Aktywny produkt to wersja webowa** (`api.py` + `frontend/`). Pakiety mobilne/desktop w `mobile_apps/` to historyczne buildy — nowe wersje desktop/mobile buduj z tego samego frontendu (wrap `frontend/dist`, np. Tauri lub Electron shell).

Szczegóły historyczne: [`mobile_apps/README_INSTALACJA.md`](mobile_apps/README_INSTALACJA.md).
...
```
moj prawnik/
├── api.py                     # Główny plik API (FastAPI, port 8003)
├── database.py                # Moduł bazy danych SQLite
├── requirements.txt           # Zależności Python
├── .env.example              # Przykładowa konfiguracja
├── cache/                    # Cache runtime (gitignore)
├── local_storage/            # Lokalny magazyn plików
│   ├── knowledge_base/       # Dokumenty prawne PDF (RAG)
│   └── chat_attachments/     # Załączniki i zdjęcia z czatu
├── scripts/                  # Uruchomienie (ULTIMATE_START.bat, sync_prompts.py)
├── supabase/migrations/      # Kanoniczne migracje Postgres
...
## Windows Desktop (web-first)

Aktywne uruchomienie:

```bat
scripts\ULTIMATE_START.bat
scripts\run_prawnik_ui.bat
```

Desktop buduj później jako wrap `frontend/dist` (Tauri/Electron). Osobne forki (`LexMind-Electron`, `LexMaind AI`) nie są utrzymywane.
