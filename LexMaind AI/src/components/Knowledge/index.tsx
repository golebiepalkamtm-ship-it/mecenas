...
export function KnowledgeView({ onBack }: { onBack?: () => void }) {
  const { documents, uploadPDF, removeFile, isUploading } = useKnowledgeBase();
        ...
        <header className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-4 w-full lg:w-auto">
                ...
                >
                  ...
                  {isUploading ? 'Wgrywanie...' : 'Dodaj Dokument'}
                </NeonButton>

                <div className="hidden sm:flex px-5 py-3 rounded-2xl bg-gold-primary/5 border border-gold-primary/20 items-center gap-3 shadow-inner">
                        ...
                        <div className="group relative glass-prestige bg-(--bg-top)/30 hover:bg-(--bg-top)/50 border-gold-muted/10 hover:border-gold-primary/30 transition-all p-5 rounded-3xl cursor-default overflow-hidden">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex flex-col items-end shrink-0 gap-2">
                                    <button
                                      onClick={(e) => { e.stopPropagation(); if(confirm(`Czy na pewno usunąć ${doc.name}?`)) removeFile(doc.name); }}
                                      className="p-2 rounded-xl text-slate-500 hover:text-red-500 hover:bg-red-500/10 transition-all opacity-0 group-hover:opacity-100"
                                      title="Usuń dokument"
                                    >
                                      <Trash2 size={16} />
                                    </button>
