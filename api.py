...
async def dedupe_database():
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            for table in tables:
                print(f"[DEDUPE] Analiza tabeli: {table}")
                url = f"{SUPABASE_URL.rstrip('/')}/rest/v1/{table}?select=id,created_at,metadata"
                res = await client.get(url, headers=headers)
                
                if res.status_code != 200:
                    print(f"[DEDUPE ERROR] Błąd pobierania danych: {res.status_code}")
                    continue
                    ...
                    if ids_to_delete:
                        ...
                        for i in range(0, len(ids_to_delete), 100):
                            ...
                            if del_res.status_code in [200, 204]:
                                ...
                                print(f"[WARN] [DEDUPE] Błąd usuwania batcha: {del_res.status_code}")
