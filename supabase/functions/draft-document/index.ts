    ...
    try {
      ...
      try {
          ...
          if (embData.data && embData.data[0]) {
              ...
              // Use ctx.supabase which automatically uses the user's auth
              const { data: matches } = await ctx.supabase.rpc('match_knowledge', {
      ...
      if (profile.subscription_tier !== 'elite') {
          ...
          await ctx.supabaseAdmin.from('profiles').update({ credits: newCredits }).eq('id', user.id);
      }
