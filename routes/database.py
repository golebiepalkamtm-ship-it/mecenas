...
async def get_all_sessions():
    ...
    return await asyncio.to_thread(database.get_sessions, limit=50)

@router.get("/sessions/{session_id}/messages")
async def get_session_messages(session_id: str, limit: int = 30):
    """Zwraca lekką historię wiadomości dla danej sesji."""
    return await asyncio.to_thread(database.get_messages, session_id=session_id, limit=limit)
...
async def get_session_message_details(session_id: str, message_id: str):
    ...
    message = await asyncio.to_thread(database.get_message_details, session_id=session_id, message_id=message_id)
    if not message:
...
async def delete_session(session_id: str):
    ...
    await asyncio.to_thread(database.delete_session, session_id)
    return {"success": True}
