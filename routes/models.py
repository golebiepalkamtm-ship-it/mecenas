...
from moa.http_client import get_ping_openai_client, get_shared_openai_client
...
async def _ping_google_native_model(model_id: str, timeout_seconds: float) -> dict:
    return {
        "status": "no_api_key",
        "error": "Google Direct integration has been disabled",
        "id": model_id,
    }
...
async def check_model_health_endpoint(model_id: str):
    ...
    if _is_google_native_model_id(model_id):
        return await _ping_google_native_model(model_id, timeout_seconds=8.0)
    ...
    try:
        ...
        if latency > 3500:
            status = "degraded"
        return {"status": status, "latency_ms": round(latency), "id": model_id}
...
async def check_models_health_bulk(req: BulkPingRequest):
    ...
    async def ping_one(mid: str):
        if _is_google_native_model_id(mid):
            return await _ping_google_native_model(mid, timeout_seconds=12.0)

        try:
            client = get_ping_openai_client(timeout_seconds=12.0)
            start_time = time.time()
            await client.chat.completions.create(
                model=mid,
                messages=[{"role": "user", "content": "p"}],
            ...
            if lat > 3500:
                status = "degraded"
            return {"id": mid, "status": status, "latency_ms": round(lat)}
        except Exception as e:
            error_str = str(e).lower()
            if "402" in error_str or "insufficient credits" in error_str:
                return {
    ...
    async def _run(mid: str):
        async with sem:
            return await ping_one(mid)

    tasks = [_run(mid) for mid in req.model_ids]
...
async def get_all_models(provider: str = "openrouter"):
    ...
    try:
        ...
        async with httpx.AsyncClient(follow_redirects=True) as client:
            ...
            if not raw_models:
                return merge_curated_models(MODELS_LIST)
        ...
        useful = merge_curated_models(useful)
...
async def get_admin_models_endpoint(
    ...
    try:
        # Pobierz wszystkie dostępne modele (tylko OpenRouter)
        all_models = await get_all_models(provider="openrouter")
        ...
        if check_status:
            ...
            # Ustaw domyślne statusy by nie blokować ładowania admina
...
async def _validate_admin_models(model_ids: list[str]) -> list[dict]:
    ...
    try:
        from routes.models import get_admin_models_endpoint
        resp = await get_admin_models_endpoint()
        all_available_models = resp.get("models", []) if isinstance(resp, dict) else resp
    except Exception:
    ...
    for model_id in model_ids:
        ...
        if model:
            ...
            # Fallback: keep the model ID even if not validated, so that selection is NEVER lost/blocked!
            valid_models.append({"id": model_id, "name": model_id.split("/")[-1] if "/" in model_id else model_id})
