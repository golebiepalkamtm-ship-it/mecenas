...
```mermaid
flowchart TD
    A[Otrzymanie aktu oskarżenia z Sądu] --> B[Złożenie pisemnej Odpowiedzi na Akt Oskarżenia]
    B --> C[Wniosek o umorzenie w trybie art. 62a u.p.n. co do posiadania]
    B --> D[Wnioski dowodowe: Allegro, PZHGP, świadkowie]
    B --> E[Wniosek o przesłuchanie funkcjonariuszy CBZC na okoliczność ważeń k. 157-162]
    C --> F[Rozprawa główna: Konfrontacja i wykazanie legalnego przeznaczenia odczynników]
    D --> F
    E --> F
    F --> G[Uniewinnienie z art. 54 i 61 u.p.n. + umorzenie / kara wolnościowa z art. 62]
```
